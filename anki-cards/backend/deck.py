...
class AnkiDeck:
    ...
    def generate_id(self) -> str:
        ...
        return f"{self.name}_{self.front}_{self.back}_{id}"

    def generate_image_tag(self, row) -> str:
    ...
    def create_notes(self) -> None:
        ...
        for _, row in self.data.iterrows():
            ...
            back = row[self.back]
            image = self.generate_image_tag(row)

            note = Note(model=self.model, fields=[front, back, image])
